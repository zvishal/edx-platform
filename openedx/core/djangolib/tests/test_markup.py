...
class FormatHtmlTest(unittest.TestCase):
    ...
    def test_simple(self, (before, after)):
        self.assertEqual(unicode(escape(_(before))), after)  # pylint: disable=translation-of-non-string
        self.assertEqual(unicode(escape(before)), after)

    def test_formatting(self):
        # The whole point of this function is to make sure this works:
        out = escape(_(u"Point & click {start}here{end}!")).format(
    ...
    def test_nested_formatting(self):
        ...
        out = escape(_(u"Send {start}email{end}")).format(
    ...
    def test_mako(self):
        ...
        template = Template(
            """
                ...
                from openedx.core.djangolib.markup import escape, HTML
                %>
                ${escape(_(u"A & {BC}")).format(BC=HTML("B & C"))}
            """,
    ...
    def test_ungettext(self):
        for i in [1, 2]:
            out = escape(ungettext("1 & {}", "2 & {}", i)).format(HTML("<>"))
