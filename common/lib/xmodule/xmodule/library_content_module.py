...
class LibraryContentModule(LibraryContentFields, XModule, StudioEditableModule):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(LibraryContentModule, self).__init__(*args, **kwargs)
    ...
    def publish_selected_children_events(cls, block_keys, format_block_keys, publish_event):
        """
        Publish selected events on child objects.  # REVIEW: Useless. Rewrite.
        Dynamically selects block_ids indicating which of the possible children are displayed to the current user.

        Arguments:

            block_keys -
                A dict describing which events to publish (add or remove), see `make_selection` above
                for format details.

            format_block_keys -
                A function to convert block keys to the format expected by publish_event. Must have the signature:

                    [(block_type, block_id)] -> T

                Where T is a collection of block keys as accepted by `publish_event`.

            publish_event -
                Function that handles the actual publishing.  Must have the signature:

                    <'removed'|'assigned'> -> result:T -> removed:T -> reason:basestring -> None

                Where T is a collection of block_keys as returned by `format_block_keys`.
        """
    ...
    def selected_children(self):
        ...
        """
        if self._selected_set is not None:
            # Already done:
