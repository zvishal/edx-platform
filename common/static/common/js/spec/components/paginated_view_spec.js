define([
    ...
    describe('PaginatedView', function () {
        var TestCardView = Backbone.View.extend({
            className: 'test-card',
            tagName: 'div',
            initialize: function () {
                this.render();
            ...
            TestPaginatedView = PaginatedView.extend({type: 'test', cardView: TestCardView}),
            testCollection,
            testView,
            initialItems,
            nextPageButtonCss = '.next-page-link',
            previousPageButtonCss = '.previous-page-link',
            generateItems = function (startIndex, stopIndex) {
        ...
        beforeEach(function () {
            ...
            initialItems = generateItems(1, 5);
         ...
         * @param expectedCards an array of topic objects we expect to see
         */
        function expectCards(expectedCards) {
            var $cards = testView.$('.test-card');
            _.each(expectedCards, function (card, index) {
                var $currentCard = $cards.eq(index);
                expect($currentCard.text()).toMatch(card.text);
            });
        }
        ...
        it('can change to the previous page', function () {
            ...
            previousPageItems = generateItems(1, 5);
            AjaxHelpers.respondWithJson(requests, {
                "count": 6,
                "num_pages": 2,
                "current_page": 1,
